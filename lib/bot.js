...
class WickrBot extends EventEmitter {
  constructor(wickr, username) {
    ...
    this.on('start', async () => await this.wickr.cmdStartAsyncRecvMessages(this.handleMessage()))
  }
  ...
  async createGroupConvo(users) {
    if (typeof users === 'string') {
      users = [users]
    }
    return await this.wickr.cmdAddGroupConvo(users)
  }

  async deleteGroupConvo(vgroupid) {
    return await this.wickr.cmdDeleteGroupConvo(vgroupid)
  }

  async createRoom(users, moderators, title, description) {
    if (typeof users === 'string') {
      users = [users]
    }
    ...
    return await this.wickr.cmdAddRoom(users, moderators, title, description || '')
  }

  async leaveRoom(vgroupid) {
    return await this.wickr.cmdLeaveRoom(vgroupid)
  }

  async deleteRoom(vgroupid) {
    return await this.wickr.cmdDeleteRoom(vgroupid)
  }
  ...
  /**
     ...
     * @returns boolean
     */
  async modifyRoom(vgroupid, users, moderators, title, description) {
    if (typeof users === 'string') {
      users = [users]
    }
    if (typeof moderators === 'string') {
    ...
    return await this.wickr.cmdModifyRoom(vgroupid, users, moderators, title, description)
  }
  ...
  /**
     ...
     * @returns boolean
     */
  async updateRoom(vgroupid, settings) {
    ...
    return await this.modifyRoom(vgroupid, members, moderators, title, description)
  }
  ...
  async getGroupConvos() {
    return JSON.parse(await this.wickr.cmdGetGroupConvos())
  }

  async getGroupConvoIDs() {
    let convos = await this.getGroupConvos().groupconvos || []
    return convos.map(c => c.vgroupid)
  }

  async getRoom(vgroupid) {
    return JSON.parse(await this.wickr.cmdGetRoom(vgroupid))
  }

  async getRooms() {
    return JSON.parse(await this.wickr.cmdGetRooms())
  }

  async getRoomIDs() {
    let rooms = await this.getRooms().rooms || []
    return rooms.map(r => r.vgroupid)
  }

  async getUsers() {
    return await this.wickr.getUsers()
  }

  async getStats() {
    return JSON.parse(await this.wickr.cmdGetStatistics()).statistics
  }

  async clearStats() {
    return await this.wickr.cmdClearStatistics()
  }

  async isConnected(timeout=10) {
    return await this.wickr.isConnected(timeout)
  }

  /**
   * send is an alias for the sendMessage method
   */
  async send(room, content, properties) {
    // not sure if there's a better way to alias this function
    return await this.sendMessage(room, content, properties)
  }
   ...
   * @returns void
   */
  async sendToUser(user, content, properties) {
    if (typeof user === 'string') {
      user = [user]
    }

    try {
      return await this.wickr.cmdSend1to1Message(user, content, '', '', '', [], JSON.stringify(properties?.meta) || '')
    } catch (error) {
   ...
   * @returns void
   */
  async sendMessage(room, content, properties) {
    try {
      return await this.wickr.cmdSendRoomMessage(room, content, '', '', '', [], JSON.stringify(properties?.meta) || '')
    } catch (error) {
      console.error('Error sending group message:', error)
    }
  }

  async sendAttachment(room, attachment, displayName) {
    try {
      return await this.wickr.cmdSendRoomAttachment(room, attachment, displayName)
    } catch (error) {
  ...
  async setAvatar(imgPath) {
    ...
    return await this.wickr.cmdSetAvatar(imgPath)
  }
